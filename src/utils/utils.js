 ...
 * @param {*} configuration 
 * @param {*} callback 
 * @return {Array} isToMerge -> true,false ; Funcs array ; isPartial -> true,false
 */
function applyMergePolicies(funcs_with_metrics, sequence_metrics, seqLen, whole, configuration, callback) {
    ...
    /**
     * 
     * @param {Array of Actions} func_list 
     * @return Outliers names
     */
    function findUpperOutliers(func_list) {

        function computeMedian(n, array) {
            return n % 2 != 0 ? array[Math.floor(n / 2)].memory : (array[Math.floor(n / 2) - 1].memory + array[Math.floor(n / 2)].memory) / 2;
        }

        var mem_usg_list = func_list.map((f) => {
            return {
                "name": f.name,
                "memory": f.limits.memory
            }
        });

        // vedi se funziona
        mem_usg_sorted = mem_usg_list.slice().sort((a, b) => {
            return a.memory - b.memory
        });

        const n = mem_usg_sorted.length;

        var median_index = Math.floor(n / 2)
        const lower = mem_usg_sorted.slice(0, median_index)
        const upper = mem_usg_sorted.slice(n % 2 != 0 ? median_index + 1 : median_index, n)

        const q1 = computeMedian(lower.length, lower);
        const q3 = computeMedian(upper.length, upper);
        const iqr = q3 - q1;
        ...
        const filteredValues = mem_usg_sorted.filter(function (x) {
            return (x.memory > maxValue);
        });

        const outliers_name = filteredValues.map((fv) => {
            return fv.name
        })

        return outliers_name;
    }
    ...
    function check(funcs) {
        ...
        if (finalCodeSize / mb > conf.LIMITS.codeSize || totalDuration > conf.LIMITS.limits.timeout || finalMemRequirement > conf.LIMITS.limits.memory) {
    ...
    if (whole) {
        ...
        const outliers = findUpperOutliers(funcs_with_metrics)
        ...
        funcs_with_metrics.forEach(f => {
             ...
             *  FIND OUTLIER: SE C'È NELLA LISTA -> FALSE
             */
            if (f.metrics.duration > f.metrics.waitTime || outliers.includes(f.name)) {
                out = false;
            }
