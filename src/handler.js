...
export async function mergeSequence(req, res) {
    ...
    const mem_policy =
        req.body.mem_policy === undefined || req.body.mem_policy === null
            ? "sum"
            : req.body.mem_policy;

    const result = await fg
        .getAction(sequenceName)
        .catch((result) => res.json("An error occurred"));
    var promises = [];
    ...
    Promise.all(promises)
        ...
        .then(async () => {
            ...
            if (sub_seq_detected) {
                ...
                res.json({
                    mex: "Sub-sequence detected, atm is not possible to optimize sequence containing other sequences!",
                });
                return;
            }

            const merged = await utils
                .merge(funcs, sequenceName, binaries_timestamp, mem_policy, true)
                .catch((result) => res.json("An error occurred"));
            res.json({ mex: "Sequence successfully merged!!", outcome: merged });
            return;
        })
        .catch((err) => {
            logger.log(err, "WARN");
...
export async function optimizeSequence(req, res) {
    ...
    const sequenceName = req.body.name;
    const isWholeMerge =
        req.body.whole === undefined || req.body.whole === null
            ? false
            : req.body.whole;
    const period =
        req.body.period === undefined || req.body.period === null
            ? false
            : req.body.period;
    const mem_policy =
        req.body.mem_policy === undefined || req.body.mem_policy === null
    ...
    Promise.all(promises)
        ...
        .then(async () => {
            ...
            const funcWithMetrics = funcs.map(async (func) => {
                ...
                const metricsRaw = await fg.getMetricsByActionNameAndPeriod(
                    func.name,
                    period
                );
            ...
            const resolvedfuncWithMetrics = await Promise.all(funcWithMetrics).catch(
                (result) => res.json("An error occurred")
            );
            const sequenceMetrics = await fg
                .getMetricsByActionNameAndPeriod(sequenceName, period)
                .catch((result) => {
                    res.json("an error occurred");
                });

            utils.applyMergePolicies(
                resolvedfuncWithMetrics,
                sequenceMetrics,
                seqLen,
                isWholeMerge,
                configuration,
                async (isToMerge, analized_funcs, isPartial) => {
                        ...
                        if (isWholeMerge) {
                            ...
                            if (!isPartial) {
                                ...
                                await utils
                                    ...
                                    .catch((error) => {
                                        logger.log("an erro occurred: "+error,"error")
                                        res.json("an error occurred ");
                                    });
                                res.json("Sequence successfully merged!!");
                                return;
                                ...
                                const resolve_sub_seq_array_parsed = await Promise.all(
                                    prom
                                ).catch((result) => {
                                    res.json("An error occurred");
                                });
                                ...
                                const final_limit = new Limit(
                                    original_limits.concurrency,
                                ...
                                var seq_names_array = [];
                                seq_names_array.push(
                                    "/_/" + resolve_sub_seq_array_parsed[0][0]
                                );
                                ...
                                for (
                                    let l = 1;
                                    l < resolve_sub_seq_array_parsed.length;
                                    l++
                                ) {
                                    seq_names_array.push(
                                        "/_/" + resolve_sub_seq_array_parsed[l][0]
                                    );
                                }
                                ...
                                fg.createActionCB(
                                    ...
                                    function (last_result) {
                                        res.json({
                                            mex: "Functions partially merged",
                                            composition: last_result,
                                        });
                                    }
                                );
                            }
...
export async function getMetrics(req, res) {
    ...
    if(Object.keys(req.body).includes("name")) res.json("Action name is required")
    let p = req.body.period === null || req.body.period === undefined ? "1d":req.body.period;

    const response = await fg
        .getMetricsByActionNameAndPeriod(req.body.name, p)
        .catch((error) => res.json("An error occurred: "+error));
